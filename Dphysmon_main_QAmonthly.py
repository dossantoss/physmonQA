...
def fdb_query(pastdatet,dfmeasure_st,del_option,range_option):
    ...
    ##rainfall tables do NOT have range check (chk_range)
    if(datum=='ra'):
    ...
    else:#all other tables HAVE range check (chk_range)   
        if(del_option ==  'filter-del' and range_option == 'filter-range' ):      
            myselect = "(SELECT " +table+ ".date_t, " +table+ "." +datum
            myfrom   = " FROM " +table
            mywhere = " WHERE del=0 and chk_range!=2 and date_t>'" +str_pastdatet+ "')"
      
        elif(del_option == 'filter-del' and range_option == 'no filter' ):      
            myselect = "(SELECT " +table+ ".date_t, " +table+ "." +datum
            myfrom   = " FROM " +table
            mywhere = " WHERE del=0 and date_t>'" +str_pastdatet+ "')"
         
        elif(del_option == 'no filter' and range_option == 'filter-range' ):      
            myselect = "(SELECT " +table+ ".date_t, " +table+ "." +datum
            myfrom   = " FROM " +table
            mywhere = " WHERE chk_range!=2 and date_t>'" +str_pastdatet+ "')" 
        else:
            myselect = "(SELECT " +table+ ".date_t, " +table+ "." +datum
            myfrom   = " FROM " +table 
            mywhere = " WHERE date_t>'" +str_pastdatet+ "')"
...
def fdb_maxquery(dfmeasure_st, del_option, range_option):
    ...
    if(datum=='ra'):
        ...
        if(del_option ==  'filter-del'and range_option == 'filter-range'):
            myselect = "(SELECT " +table+ ".date_t, " +table+ "." + datum
            myfrom   = " FROM " +table
            mywhere = " WHERE del=0 and chk_range!=2)"   
            #Fetch date_t and datum-values from physmon table
            dbquery = myselect + myfrom + mywhere 
...
def fmonth_stats(dfphysmon_stats):
    ...
    dfphysmon_stats = dfphysmon_stats[dfphysmon_stats['records']>671]
...
def update_figure(yaxis_series, del_option, range_option, xaxis_timespan): 
    ...
    try:
       ...
       if xaxis_timespan == 'one month':
                ...
                mytbquery = fdb_query(pastdatet,dfphysmon_list, del_option, range_option) #function call
                ...
                #Fetch date_t and datum-values from physmon table
                mytbquery = fdb_maxquery(dfphysmon_list, del_option, range_option) #function call
